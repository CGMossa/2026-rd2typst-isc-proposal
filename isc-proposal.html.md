...
`R CMD` already ships a small family of Rd converters:
...
This proposal adds two more: **`Rd2typst`** (or **`Rd2typ`** for
alignment with the `Rd2pdf` / `Rd2txt` naming convention; final name
to be agreed with R-core) and **`Rd2md`**. Both fit naturally into the
existing tooling surface and do not require new R CMD entry points
