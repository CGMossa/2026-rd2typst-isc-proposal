...
-   `Rd2typst()` produces valid output for at least 95% of CRAN
...
-   Updated benchmark extending the existing 257-page, 1.9x-faster
    baseline to a broader CRAN-package corpus
...
-   CI matrix (GitHub Actions) runs both backends on Windows, macOS,
...
-   Output validates against an LLM context-window test: a
    representative help page parses without encoding artifacts or broken
    structure in a standard context window
...
| Rd2md output validates               | Parses in LLM context window without errors | Manual + automated test |
...
-   **`R CMD Rd2html --backend=typst`:** M1 and M2 are the necessary
    foundation for a Typst-based HTML backend for R help pages. Typst's
    HTML output supports native typesetting of equations and
    bibliographies. The R community pioneered reproducible scientific
    writing through Sweave, knitr, R Markdown, and Quarto. A
    Typst-based HTML backend is the natural next step in that legacy.
    M1 delivers a CRAN-validated converter. M2 delivers the embedded
    template and binary management. Anyone, the author or another
    contributor, can build the HTML backend on this foundation.
