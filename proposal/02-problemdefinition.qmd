...
r-source. There is no CTAN-equivalent fragility. There is no user-side
...
## The LaTeX dependency is painful

R's documentation system is one of its great strengths: every function
ships with structured, cross-referenced help that can be rendered to
HTML, plain text, or PDF. The PDF path, `tools::Rd2pdf`, has served the
community well for decades. It relies entirely on LaTeX, a dependency
that has grown increasingly painful.
...
failures. When a CRAN package depends on LaTeX packages that ship in
...
The benchmark in `code/typst-benchmark.R` measures the Typst backend at
...
the author's r-source fork. It handles the full set of Rd block and
...
The key gap is not the Rd to Typst conversion or the CLI integration.
